...
{
    public class StructureBubbleHelper
    {
        public event EventHandler<StructureBubbleStorageItem>? OnStructureChange;
        ...
        public void Init(int throtlingms = 5000, double minDistanceUpdateBorder = 100)
        {
            ...
            HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(Init), 0, $"Init => minDist:{minDistanceUpdateBorder}");
        }
        ...
        public async Task<List<StructureStorageItem>> SetMinDistance(double minDistanceUpdateBorder, string jsonListBars)
        {
            ...
            try
            {
                ...
                HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(SetMinDistance), 0, $"SetMinDistance => minDist:{minDistanceUpdateBorder} | jsonListBars: {jsonListBars}");
            ...
            HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(SetMinDistance), 0, $"_lastStructure: {structures.Count}");
        ...
        StructureBubbleStorageItem? _lastStructure { get; set; }
        ...
        {
            //HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(Init), 0, $"OnNewBar");
            //se for primeiro candle
            if (_lastStructure == null)
            {

                _lastStructure = new StructureBubbleStorageItem()
                {
                ...
                HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(OnNewBar), 0, $"First _lastStructure: {_lastStructure.ToString()}");
            ...
            //HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(OnNewBar), 0, $"MinDist:{_minDistanceUpdateBorder} | Diff Up|Close: {virtualSameUpAuxBorder - newBar.Close}");
            ...
            //HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(OnNewBar), 0, $"MinDist:{_minDistanceUpdateBorder} | Diff Down|Close: {newBar.Close - virtualSameDownAuxBorder}");
            ...
            HelperPerformanceConfig.Log(nameof(StructureBubbleHelper), nameof(OnNewBar), 0, $"UpAuxBorder: {_lastStructure.UpAuxBorder.ToString("0.00")} | UpBorder: {_lastStructure.UpBorder.ToString("0.00")}");
